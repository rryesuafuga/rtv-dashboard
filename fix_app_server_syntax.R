...
# fix_app_server_syntax.R - Fix syntax error in app_server.R

cat("Fixing syntax error in app_server.R...\n\n")

# Create a clean app_server.R file
app_server_content <- '# R/app_server.R - Server Configuration
...
writeLines(app_server_content, "R/app_server.R")
cat("✓ Fixed syntax error in app_server.R\n")

# Also check if all required libraries are loaded
cat("\nChecking if readr is loaded for read_csv...\n")

if (file.exists("app.R")) {
  app_content <- readLines("app.R", warn = FALSE)
  
  # Check if readr is loaded
  if (!any(grepl("library\\(readr\\)|library\\(tidyverse\\)", app_content))) {
    cat("Adding readr library to app.R...\n")
    
    # Find where libraries are loaded
    lib_line <- max(grep("^library\\(", app_content))
    
    # Add readr after other libraries
    new_content <- c(
      app_content[1:lib_line],
      "library(readr)",
      app_content[(lib_line+1):length(app_content)]
    )
    
    writeLines(new_content, "app.R")
    cat("✓ Added readr library\n")
  }
}
